...
import { Plus, LogOut, ExternalLink, Trophy } from "lucide-react";
...
export default function TeacherDashboard() {
  ...
  async function load() {
    if (!user) return;
    const { data } = await supabase.from("sessions").select("*").eq("teacher_id", user.id).order("created_at", { ascending: false });
    setSessions(data || []);
  ...
  async function createSession() {
    ...
    // Seed 5 challenge rows with sensible defaults so the teacher can edit
    ...
    <div className="app-shell pb-16">
      ...
      <div className="px-4 space-y-4">
        <div className="flex gap-2">
          <button onClick={createSession} className="btn-primary flex items-center justify-center gap-2">
            <Plus className="w-5 h-5" /> New Session
          </button>
          <button onClick={logout} className="px-4 rounded-full border-2 border-border text-muted-foreground">
          ...
          return (
            <div key={s.id} className="app-card space-y-3">
              <div className="flex items-start justify-between gap-2">
                ...
                <Link to={`/teacher/session/${s.id}`} className="text-action font-semibold flex items-center gap-1 text-sm">
              ...
              <label className="flex items-center justify-between text-sm bg-muted/50 rounded-xl px-3 py-2">
                ...
                <input type="checkbox" checked={s.allow_late_registration} onChange={() => toggleLate(s)} className="w-5 h-5 accent-[hsl(var(--action))]" />
              </label>

              <div className="space-y-2">
                <div className="flex items-center justify-between text-sm">
                  <span className="font-semibold text-primary">Live Leaderboard</span>
                  <span className="text-xs text-muted-foreground">{sGroups.length} groups · {completed} done</span>
                </div>
                {sGroups.length === 0 && (
                ...
                {sGroups
                  ...
                  .map((g) => {
                    ...
                    const elapsed = g.start_time
                      ? Math.round(((g.finish_time ? +new Date(g.finish_time) : Date.now()) - +new Date(g.start_time)) / 1000)
                      : 0;
                    return (
                      <div key={g.id} className={`rounded-xl p-3 border ${g.finish_time ? "border-success bg-success/5" : "border-border bg-background/40"}`}>
                        <div className="flex items-center justify-between text-sm">
                          <span className="font-semibold">{g.group_name} {g.finish_time && <Trophy className="inline w-4 h-4 text-success ml-1" />}</span>
                          <span className="text-xs text-muted-foreground">L{g.current_level}/5 · {Math.floor(elapsed / 60)}m {elapsed % 60}s</span>
                        </div>
                        <div className="h-2 bg-muted rounded-full mt-2 overflow-hidden">
                          <div className="h-full bg-action transition-all" style={{ width: `${Math.min(100, pct)}%` }} />
                        </div>
                      </div>
                    );
                  })}
